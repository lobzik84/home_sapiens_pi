...
class CGSMAlphabets
{
	...
	//private static final String alphabet = "@£$¥@@@@@@@@@@@@Ä_ÖÃËÙÐØÓÈÎ@@@@@ !\"#€%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ@@@@§?abcdefghijklmnopqrstuvwxyz@@@@@";
	...
	*/
	protected static String char2Hex(char c, int charSet) {
		...
		//Patched by Pino Navato: unknown chars are rendered with a '?'
		return char2Hex('?', charSet);
	}
...
//				Patched by Pino Navato: accented vocals and other "special" characters were not handled correctly!
...
*/
	...
	*/
	...
	protected static String text2Text(String text, int charSet) {
		String outText = "";
		boolean lastWas27 = false;
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (c == 27) {
				...
				if (lastWas27) {
					outText += hex2ExtChar((char) c, charSet);
					lastWas27 = false;
				} else {
					outText += hex2Char((char) c, charSet);
				}
			} 
		}
		return outText;
